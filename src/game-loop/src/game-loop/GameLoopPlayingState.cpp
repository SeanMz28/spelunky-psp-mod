...
void GameLoopPlayingState::enter(GameLoop& game_loop)
{
    ...
    extern const std::string& get_custom_level_path();

auto& tb = Level::instance().get_tile_batch();

const auto& custom = get_custom_level_path();
if (!custom.empty() && tb.load_custom_level_from_txt(custom)) {
    tb.generate_frame();
    tb.generate_cave_background();
    tb.batch_vertices();
    tb.add_render_entity(registry);
} else {
    TileBatch::LevelGeneratorParams generator_params;
    generator_params.shopkeeper_robbed = game_loop._shopping_system->is_robbed();

    tb.generate_new_level_layout(generator_params);
    tb.initialise_tiles_from_room_layout();
    tb.generate_frame();
    tb.generate_cave_background();
    tb.batch_vertices();
    tb.add_render_entity(registry);
