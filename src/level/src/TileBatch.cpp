...
static MapTileType char_to_type(char c) {
    switch (c) {
        case '1': return MapTileType::CAVE_REGULAR; // solid walls
        case 'L': return MapTileType::LADDER;
        case 'D': return MapTileType::ENTRANCE;     // player spawn
        case 'E': return MapTileType::EXIT;
        case '0':
        default:  return MapTileType::NOTHING;
    }
}

bool TileBatch::load_custom_level_from_txt(const std::string& path) {
    ...
    const int H = std::min<int>((int)lines.size(), Consts::LEVEL_HEIGHT_TILES);
    const int W = std::min<int>((int)lines[0].size(), Consts::LEVEL_WIDTH_TILES);

    for (int y = 0; y < H; ++y) {
        // safety if lines are ragged
        const int rowW = std::min<int>((int)lines[y].size(), Consts::LEVEL_WIDTH_TILES);
        for (int x = 0; x < rowW; ++x) {
            auto t = char_to_type(lines[y][x]);
            map_tiles[x][y]->map_tile_type = t;
            map_tiles[x][y]->x = x;
            map_tiles[x][y]->y = y;
            map_tiles[x][y]->match_tile(t); 
            // flags like .climbable/.destroyable are inferred elsewhere from type,
            // and many systems check map_tile_type directly (see Bomb.cpp), so this is enough.
        }
    }
    return true;
}
